...
export default function MasjidDisplay() {
  ...
  // 3. Setup active slides
  useEffect(() => {
    if (settings) {
      let active = (settings.rotation_pages || []).filter(p => p.active);
      
      // Inject posters as individual slides
      if (settings.posters && settings.posters.length > 0) {
